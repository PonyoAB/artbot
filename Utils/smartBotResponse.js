...
// Set the color of the embed
    .setColor(ARTBOT_GREEN)
// Set the main content of the embed
    .setDescription(`It looks like you're wondering about when the next drop is.\n\nFor details on upcoming scheduled releases, please check the [#upcoming-projects](https://discord.com/channels/411959613370400778/872986185167949885/878001308962000936) channel.\n
...
async function generateGasPriceMessage() {
  const gasStationResponse = await fetch(`https://ethgasstation.info/api/ethgasAPI.json?api-key=${GASSTATION_API_KEY}`);
...
async function smartBotResponse(msgContentLowercase, msgAuthor, artBotID, channelID) {
  ...
  const mentionedArtBot = msgContentLowercase.includes(ARTBOT_USERNAME) ||
  ...
  const mentionsPause = msgContentLowercase.includes('pause') ||
  ...
  /*
     ...
     */
  const messageMentionsSquiggle = msgContentLowercase.includes('squiggle') ||
  ...
  const mentionedCuratedPlaygroundFactory = msgContentLowercase.includes('curated') ||
